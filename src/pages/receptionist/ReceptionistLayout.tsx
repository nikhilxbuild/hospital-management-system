...
import { LayoutDashboard, Users, UserPlus, LogOut, Heart, ChevronLeft, ChevronRight } from "lucide-react";

const links = [
  { label: "Dashboard", path: "/receptionist/dashboard", icon: LayoutDashboard },
  { label: "Today's Queue", path: "/receptionist/queue", icon: Users },
  { label: "Add Walk-In", path: "/receptionist/walkin", icon: UserPlus },
];

const ReceptionistLayout = () => {
  const { session, logout } = useStaffSession("receptionist");
          ...
          {links.map((l) => (
            <Link key={l.path} to={l.path}
              className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors ${
                location.pathname === l.path ? "bg-sidebar-accent text-sidebar-primary" : "text-sidebar-foreground/70 hover:bg-sidebar-accent"
              }`}>
